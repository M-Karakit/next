        ...
        <div className={`${isMenuOpen ? 'flex flex-col gap-10' : 'hidden'} md:flex md:flex-row md:justify-between items-center md:gap-8 lg:gap-10 xl:gap-20 2xl:gap-41.75`}>
            <ul className="flex flex-col md:flex-row items-center gap-7 md:gap-4 lg:gap-8 2xl:gap-10">
                {navProps.links.map((link, index) => (
                    <li key={index}>
                        <Link href={link.path} className={`text-xl md:text-sm xl:text-base 2xl:text-xl font-open-sans font-semibold leading-none ${pathName === link.path ? 'pb-0.5 border-b border-[#FA8B02]' : ''}`}>{link.name}</Link>
                    </li>
                ))}
            </ul>
            <div className="flex gap-3 md:gap-1.5 xl:gap-2.5">
                ...
                <div className="flex gap-2.5">
                    {navProps.buttons.map((btn, index) => (
                        <button key={index} className={`md:px-1 2xl:py-2.5 xl:px-3 md:text-sm xl:text-base 2xl:text-xl font-open-sans font-semibold leading-none rounded-full ${btn.isActive ? 'bg-[#FA8B02] px-5 py-2 md:px-3 md:py-1.5 lg:px-4 2xl:px-6 lg:w-24 lg:h-10 2xl:w-42 2xl:h-11.75' : ''}`}>{btn.text}</button>
                    ))}
                </div>
            </div>
        </div>
